...
$(document).ready(() => {
  ...
  ctx = canvas.getContext('2d'); // FIGURE OUT WHAT THE CONTEXT ID IS
...
const paintToggle = () => {
  ...
  for(let i=0; i<TOGGLE_COLS; i++) {
    ...
    ctx.lineTo(i*TILE_WIDTH,TILE_HEIGHT*TOGGLE_ROWS);
  }
  // TAKES CARE OF HORIZONTAL LINES
  for(let i=0; i<TOGGLE_ROWS; i++) {
    ctx.moveTo(0,i*TILE_HEIGHT);
    ctx.lineTo(TILE_WIDTH*TOGGLE_COLS,i*TILE_HEIGHT);
  }
  ctx.stroke();
